...
/**
 * print_magic - Print the amgic number of an ELF header.
 * @e_ident: Pointer to an array containing elf magic numbers.
 *
 * Description: Magic number are separated with spaces.
 */
void print_magic(unsigned char *e_ident)
{
	int index;
 ...
 * @e_ident: Pointer to an array containing the ELF class.
 * @e_entry: Address of the ELF entry entry point.
 */
void print_entry(unsigned long int e_entry, unsigned char *e_ident)
{
 ...
 *
 * Descriptor: If the file cannot closed - exit code 98.
 */
void close_elf(int elf)
{
	if (close(elf) == -1)
